...
```{r}
planets <- data.frame(type = factor(type), diameter, rotation, rings, row.names = planet)
```
...
```{r}
rm(planet, type, diameter, rotation, rings)
```
...
```{r}
plot(planets$diameter, planets$rotation)
```
...
```{r include=T, echo=T}
planets_list <- list(planets       = planets, 
                     type_levels   = levels(planets$type),  
                     mean_rotation = mean(planets$rotation))
```
...
```{r}
planets_list$planets$rotation[c(2, 4)]
```
